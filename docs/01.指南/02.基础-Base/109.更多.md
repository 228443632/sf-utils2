...
### 17 getSize(obj) 返回不同类型的的长度（对象/数组/Map/Set/String） <Badge text="v3.3+" type="success" vertical="middle"/>
```javascript
getSize([1, 2, 3]); // 3
...
### 18 unserialize(obj) 反转序列化查询参数 <Badge text="v3.3+" type="success" vertical="middle"/>
```javascript
unserialize('id=123&name=test1') => {id: '123', name: 'test1'}
```


### 19 serialize(obj) 序列化查询参数 <Badge text="v3.3+" type="success" vertical="middle"/>
```javascript
