...
```javascript
import { domUtils } from '@bianpengfei/utils'
...
::: warning
该方法已从 <Badge text="3.0.14+" type="success" vertical="middle"/> 后迁移到了_helpEventBus文件中，
名称由 `eventBus` 更名为 `_helpEventBus`
:::

```javascript
// ⚠️ 该方法已从3.0.14+后迁移到了_helpEventBus文件中
import { _helpEventBus } from '@bianpengfei/utils'
