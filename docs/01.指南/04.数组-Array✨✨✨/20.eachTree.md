...
## 1.3 示例 先子节点到父节点 递归遍历【`莫发达`提供】

> 注：以前我没注意 都是从父到子进行递归，先子节点到父节点 递归遍历【`莫发达`提供】，默认是先父节点到子节点。
...
<br/>
输出结果
<img :src="$withBase('/img/console.log-24.png')">
...
| traversalOrder | 树的遍历顺序 <Badge text="v3.2.4+" type="success" vertical="middle"/>，感谢`莫发达`童鞋提出意见 | `preorder`（先进行父->子 默认）、`postorder`（后进行子->父） | 否       | preorder |
